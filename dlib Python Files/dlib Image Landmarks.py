...
for (i, rect) in enumerate(rects):
    ...
    # convert dlib's rect to an OpenCV bounding box
    (x, y, w, h) = face_utils.rect_to_bb(shape)
    ...
    # loop over the (x, y) cords for the facial landmarks
    # and draw them on the image
    for (x, y) in shape:
        cv2.circle(image, (x, y), 1, (0, 255, 0), -1)

# show the output image with face detections + facial landmarks
